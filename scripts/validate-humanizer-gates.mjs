...
function validateGate1() {
  ...
  for (const claim of ledger.claims ?? []) {
    ...
    if (['publish', 'qualified', 'reported', 'current_fact', 'dated_fact', 'listed_status_open', 'conflicted', 'expired_or_historical'].includes(claim.status)) {
      ...
      if (claim.status === 'reported') expect(Boolean(claim.uncertainty || claim.reason), `Reported claim ${claim.id} has no uncertainty note.`);
