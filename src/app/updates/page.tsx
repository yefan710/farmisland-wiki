...
import { formatGameDate, site } from "@/data/site";
...
export default function UpdatesPage() { return <>
  ...
  <PageHero eyebrow="Game updates" title="No patch details are listed yet" answer={`The Farm an Island game record changed on ${formatGameDate(site.lastGameUpdate)}. Its public description does not include a list of features or balance changes. Open the live experience for the newest game state.`} nextHref="#latest" nextLabel="See the latest date" status={`Update record checked ${formatGameDate(site.updateCheckedAt)}`} />
  <section className="section"><div className="page-shell content-layout"><div>
    <article className="content-panel" id="latest"><span className="eyebrow">Latest date</span><h2>The game record shows {formatGameDate(site.lastGameUpdate)}</h2><p>The update timestamp still does not explain what changed inside the game, so there is no feature or balance list on this page yet.</p><p className="player-note">Use the official experience as the final check for current gameplay.</p></article>
    <article className="content-panel" id="snapshot"><span className="eyebrow">Official snapshot</span><h2>Dated public totals</h2><table className="data-table"><thead><tr><th>Field</th><th>Value</th><th>Checked</th></tr></thead><tbody><tr><td>Playing</td><td>{site.playing.toLocaleString("en-US")}</td><td>{formatGameDate(site.checkedAt)}</td></tr><tr><td>Favorites</td><td>{site.favorites.toLocaleString("en-US")}</td><td>{formatGameDate(site.checkedAt)}</td></tr><tr><td>Visits</td><td>{site.visits.toLocaleString("en-US")}</td><td>{formatGameDate(site.checkedAt)}</td></tr></tbody></table><p>These totals are a snapshot and can change after the check.</p></article>
